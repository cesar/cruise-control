...
void writeDataPins(uint8_t db7, uint8_t db6, uint8_t db5, uint8_t db4, uint8_t db3, uint8_t db2, uint8_t db1, uint8_t db0) {
  ...
  //DB7
  GPIOPinWrite(port_C, GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 |GPIO_PIN7,

    db7 * pin_7 + db6 * pin_6 + db5 * pin_5 + db4 * pin_4);

  GPIOPinWrite(port_D, GPIO_PIN_0| GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3, db3*pin_3 + db2*pin_2 + db1*pin_1 + db0*pin_0 );
...
void enableHL() {
  ...
  //Enable high
  GPIOPinWrite(port_D, GPIO_PIN_6, pin_6);
  
   //Wait 5ms
  SysCtlDelay(130000);
  
  //Enable low
  GPIOPinWrite(port_D, GPIO_PIN_6, 0x0);
...
void initializeDisplay() {
  ...
  //RS low
  GPIOPinWrite(port_A, GPIO_PIN_5, 0x0);
...
void write_phrases()
{
  ...
  //Move list down
    ...
    if(flag_check == 1){
      ...
      { 
        GPIOPinWrite(port_A, GPIO_PIN_5, pin_5);
